...
/**
 * Glyph Matrix Toy Service for previous track media control.
 * 
 * Features:
 * - Long press triggers previous track command
 * - Visual feedback with direction-aware animations
 * - Shared theme system with NextTrackToyService
 * - State-based rendering (idle, pressed, long-pressed)
 */
class PreviousTrackToyService : GlyphMatrixService("PreviousTrack-Toy") {
    
    companion object {
        private const val TAG = "PreviousTrackToyService"
    ...
    override fun performOnServiceConnected(
        ...
        Log.d(TAG, "PreviousTrackToyService connected")
    ...
    override fun performOnServiceDisconnected(context: Context) {
        Log.d(TAG, "PreviousTrackToyService disconnected")
    ...
    override fun onTouchPointLongPress() {
        Log.d(TAG, "Long press detected - triggering previous track")
        ...
        serviceScope.launch {
            try {
                val success = mediaHelper.skipToPrevious()
                if (success) {
                    Log.d(TAG, "Successfully triggered previous track")
                    
                    // Keep visual feedback for a moment
                    delay(LONG_PRESS_FEEDBACK_DURATION)
                } else {
                    Log.w(TAG, "Failed to trigger previous track")
                }
            } catch (e: Exception) {
                Log.e(TAG, "Error triggering previous track", e)
            } finally {
    ...
    private fun updateDisplay(state: TrackControlTheme.InteractionState) {
        ...
        val frame = theme.getStateFrame(
            state = state,
            direction = TrackControlTheme.Direction.PREVIOUS,
